...
namespace PostSharp.Toolkit.Threading.Dispatch
{
    ...
    public sealed class DispatchMethodAttribute : MethodInterceptionAspect
    {
        ...
        public override bool CompileTimeValidate( MethodBase method )
        {
            if ( method.IsStatic )
            {
                Message.Write( method, SeverityType.Error, "THREAD01", "Cannot apply DispatchMethodAttribute to method {0}.{1}: the method is static.",
                               method.DeclaringType.Name, method.Name );
                return false;
            }
            else if ( !typeof(DispatcherObject).IsAssignableFrom( method.DeclaringType ) )
            {
                Message.Write( method, SeverityType.Error, "THREAD02",
