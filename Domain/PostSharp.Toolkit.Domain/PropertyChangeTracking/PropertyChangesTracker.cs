...
namespace PostSharp.Toolkit.Domain.PropertyChangeTracking
{
    internal static class PropertyChangesTracker
    {
        ...
        public static void RaisePropertyChangedOnlyOnSpecifiedInstance( object instance )
        {
            ...
            RaisePropertyChangesInternal( propertyChangesAcumulator.Value, false, w => w.Instance.IsAlive && ReferenceEquals( w.Instance.Target, instance ) );
        }
        ...
        {
            RaisePropertyChangesInternal( childPropertyChangesAcumulator.Value, true, w => w.Instance.IsAlive );
            //&& (!stackTrace.Value.Contains(w.Instance.Target) || ReferenceEquals(w.Instance.Target, source)));
        }
